...
const Index = () => {
  ...
  useEffect(()=> {
    if (currentComponent === 1 && audio1Ref.current) {
      ...
      audio1Ref.current.play().catch((err) => console.log('audio play block (countdown):', err));
    } else if (currentComponent > 1 && audio2Ref.current) {
      audio1Ref.current?.pause();
      audio1Ref.current.currentTime = 0;

      audio2Ref.current.play().catch((err) => console.log('audio play block (background):', err));
    }
  },[currentComponent])
  ...
  const renderCurrentComponent = () => {
    switch (currentComponent) {
      case 1:
        return <CountdownComponent onComplete={nextComponent} audioRef={audio1Ref} />;
      case 2:
    ...
    {/* Start Screen */}
    {
      currentComponent === 0 ? (
        <div className="relative z-10 min-h-screen flex items-center justify-center">
          <motion.button onClick={()=> setCurrentComponent(1)} 
            className="'px-12 py-4 text-lg font-semibold text-white bg-gradient-romantic rounded-full shadow-romantic hover:shadow-glow transform transition-all duration-300"
        ...
        // Main Content
        <div className="relative z-30 h-screen flex items-center juc p-4">
